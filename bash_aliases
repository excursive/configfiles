...
batch_optimize_files() {
  ...
  for in_file in "$@"; do
    ...
    case "$filetype" in
      ...
      ;;
      'pngstrip')
        zopflipng -y --keepchunks=PLTE,tRNS,cHRM,gAMA,iCCP,sBIT,sRGB,bKGD,hIST,pHYs,sPLT,tIME "${in_file}" "${temp_file}"
      ;;
      'pngmstrip')
        zopflipng -m -y --keepchunks=PLTE,tRNS,cHRM,gAMA,iCCP,sBIT,sRGB,bKGD,hIST,pHYs,sPLT,tIME "${in_file}" "${temp_file}"
      ;;
      'pngmncstripall')
